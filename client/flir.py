...
class Flir:
    def __init__(self, baseURL='http://169.254.101.173/'):
        self.baseURL = baseURL

    def setResource(self,resource,value):
        with urllib.request.urlopen(self.baseURL+'res.php',urllib.parse.urlencode({'action':'set','resource':resource,'value':value}).encode("utf-8")) as f:
            return f.read()

    def getResource(self,resource):
        with urllib.request.urlopen(self.baseURL+'res.php',urllib.parse.urlencode({'action':'get','resource':resource}).encode("utf-8")) as f:
            return f.read()
